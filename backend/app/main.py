"""
FastAPI Backend for ATLAS Protocol System
...
# TEMPORARY: Disabled routers with missing dependencies
# from .api.documents_coverage import router as coverage_router, set_rag_engine as set_coverage_engine
# from .api.models import router as models_router
from .api.model_control import router as model_control_router, set_model_manager, set_rag_engine as set_model_control_engine
...
async def lifespan(app: FastAPI):
    ...
    Initializes ModelManager and RAG engine on startup and cleans up on shutdown.
    """
    global rag_engine, model_manager

    # Startup
    logger.info("=" * 70)
    logger.info("STARTING ATLAS PROTOCOL FASTAPI BACKEND")
    logger.info("=" * 70)
    ...
    try:
        ...
        # Step 2: Initialize ModelManager with hotswappable models
        logger.info("Initializing ModelManager with hotswappable LLM models...")
        from _src.model_manager import ModelManager

        model_manager = ModelManager(config)

        # Select model based on configured LLM backend
        if config.llm_backend.lower() == "llamacpp":
            default_model_id = "llama-8b-q5"  # llama.cpp: 80-100 tok/s
            logger.info(f"Using llama.cpp backend: {default_model_id} (8-10x faster than Ollama)")
        else:
            default_model_id = "ollama"  # Ollama HTTP: 8-12 tok/s
            logger.info(f"Using Ollama HTTP backend: {default_model_id}")

        await model_manager.initialize(default_model_id=default_model_id)

        # Set manager in model_control router
        set_model_manager(model_manager)
        logger.info("✓ ModelManager ready with model hotswapping enabled")

        # Step 3: Initialize RAG engine with ModelManager
        logger.info("Initializing RAG Engine...")
        rag_engine = RAGEngine(config)

        # Inject ModelManager into RAG engine
        rag_engine.model_manager = model_manager
        rag_engine.llm = model_manager.get_llm()  # Get initial LLM from manager
        ...
        # TEMPORARY: Disabled routers
        # set_coverage_engine(rag_engine)
        set_model_control_engine(rag_engine)  # For model hotswap synchronization
...
app = FastAPI(
    ...
    description="""
    FastAPI backend for ATLAS Protocol Document Intelligence System.
...
app.include_router(model_control_router)  # Model hotswapping endpoints
