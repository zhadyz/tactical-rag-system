...
export class ErrorBoundary extends Component<Props, State> {
  ...
  logErrorToFile = async (error: Error, errorInfo: ErrorInfo) => {
    try {
      ...
      try {
        ...
        const logsString = JSON.stringify(trimmedLogs);
        const estimatedSize = logsString.length;

        // localStorage quota is usually 5-10MB, be conservative with 512KB limit for error logs
        if (estimatedSize > 512 * 1024) {
          // Clear old logs and keep only last 10
          const minimalLogs = trimmedLogs.slice(-10);
          localStorage.setItem('error-logs', JSON.stringify(minimalLogs));
        } else {
          localStorage.setItem('error-logs', logsString);
        }
        ...
        if (storageErr.name === 'QuotaExceededError') {
          try {
            // Clear error logs and try again with just the current error
            localStorage.removeItem('error-logs');
            localStorage.setItem('error-logs', JSON.stringify([errorLog]));
            localStorage.setItem('latest-error', JSON.stringify(errorLog));
          } catch {
            // Give up gracefully
          }
  ...
  exportErrorLogs = () => {
    try {
      ...
      a.href = url;
      a.download = `forge-errors-${new Date().toISOString()}.json`;
