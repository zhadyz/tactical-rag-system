...
description: Comprehensive guide to diagnosing and fixing common issues in Forge
---
...
Comprehensive guide for diagnosing and resolving common issues with Forge.

## Troubleshooting Overview

Forge is a complex system with multiple layers (Frontend, Tauri, Backend). Issues can arise at any layer. This guide helps you:
...
docker ps
docker logs forge-backend --tail 50
...
# View detailed logs
docker-compose -f backend/docker-compose.yml logs forge-backend
...
docker exec forge-backend nvcc --version

# Check GPU visibility
docker exec forge-backend nvidia-smi

# Verify CUDA libraries
docker exec forge-backend ldconfig -p | grep cuda
```
...
docker logs forge-backend -f | grep -i "loading\|model\|initialized"

# Check VRAM during load
watch -n 1 nvidia-smi
```

**Fix**: Increase health check `start_period` in `docker-compose.yml`:
```yaml
healthcheck:
...
watch -n 5 'docker stats --no-stream | grep forge-backend'

# Check Python memory usage
docker exec forge-backend python -c "
import psutil
...
```bash
docker-compose down
docker-compose build --no-cache forge-backend
docker-compose up -d
```
...
docker exec forge-backend redis-cli -h redis ping
...
docker exec forge-backend netstat -tuln | grep 8000
```
...
```bash
docker logs forge-backend | jq 'select(.level == "ERROR")'
docker logs forge-backend | jq 'select(.component == "llm_engine")'
```
...
- **Backend logs**:
   ```bash
   docker logs forge-backend --tail 200 > backend.log
   ```
...
- **Discord**: [Link]
- **Documentation**: [https://docs.forge-rag.com](/)
- **Email**: z@onyxlab.ai
