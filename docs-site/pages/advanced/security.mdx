...
description: Comprehensive security measures and best practices for Forge system
---
...
Forge implements a **defense-in-depth security strategy** across all layers of the application. This guide covers security measures, threat mitigation, and production hardening.

---

## Security Overview

Forge's security architecture follows **OWASP best practices** with multiple layers of protection:
...
<Callout type="warning">
  **Local-Only by Default**: Forge is designed for localhost-only deployment. Production internet exposure requires additional hardening (see Production Checklist).
</Callout>
...
Forge detects **20+ suspicious patterns** that could indicate prompt injection attacks:
...
Forge implements **dual-tier rate limiting**:
...
Forge uses **Qdrant (vector DB)** and **Redis (cache)**, neither of which uses SQL. All queries use parameterized APIs.
...
Forge runs **locally without authentication**. No API keys, passwords, or tokens are required.
...
Before deploying Forge to production, complete this checklist:
