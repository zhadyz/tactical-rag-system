...
description: Comprehensive guide to monitoring Forge performance, health, and system metrics for production deployments
---
...
Forge includes built-in monitoring and observability features designed for production deployments. This guide covers metrics collection, health checks, performance profiling, and alerting strategies.

## Monitoring Overview

Forge provides multi-layered observability across three critical dimensions:
...
<Callout type="info">
**Production Readiness**: Forge uses structured JSON logging and rotating file handlers for offline analysis and field debugging.
</Callout>
...
Forge automatically tracks performance metrics across the entire request pipeline:
...
Forge's `StageTimer` tracks timing at each pipeline stage:
...
Forge uses structured JSON logging for production deployments:
...
```bash
logs/
├── forge-protocol.log          # All logs (rotating, 100MB per file, 10 backups)
├── forge-protocol-errors.log   # Errors only (rotating, 50MB per file, 5 backups)
...
Forge is designed for Prometheus integration:
...
# Query metrics
query_total = Counter('forge_queries_total', 'Total queries processed', ['mode', 'cache_hit'])
query_latency = Histogram('forge_query_latency_seconds', 'Query latency', ['mode'])
cache_hit_rate = Gauge('forge_cache_hit_rate', 'Cache hit rate')

# System metrics
gpu_memory_used = Gauge('forge_gpu_memory_mb', 'GPU memory usage in MB')
active_connections = Gauge('forge_active_connections', 'Active connections')
```
...
{
  "dashboard": {
    "title": "Forge Monitoring",
    "panels": [
      {
        "title": "Query Latency (p50, p95, p99)",
        "targets": ["forge_query_latency_seconds"],
        "type": "graph"
      },
      {
        "title": "Cache Hit Rate",
        "targets": ["forge_cache_hit_rate"],
        "type": "stat"
      },
      {
      ...
      },
      {
        ...
        "targets": ["forge_component_status"],
        "type": "stat"
      }
    ]
  }
...
- Review logs: `tail -f logs/forge-protocol-errors.log`
