...
description: Deploy Forge with Docker Compose, multi-stage builds, and GPU acceleration
---
...
Deploy Forge to production with Docker Compose orchestration, GPU acceleration, and production-grade monitoring.
...
Forge's production deployment uses a **multi-container architecture** orchestrated via Docker Compose:

- **forge-backend**: FastAPI + RAG pipeline with GPU acceleration
...
The complete stack is defined in `backend/docker-compose.yml`:
...
Forge uses a **5-stage Dockerfile** for optimal caching and minimal image size (9GB, 52% reduction from baseline).
...
docker exec forge-qdrant \
  curl -X POST http://localhost:6333/collections/forge_documents/snapshots

# Copy snapshot
docker cp forge-qdrant:/qdrant/snapshots ./backups/qdrant/

# Backup Redis RDB
docker exec forge-redis redis-cli BGSAVE
docker cp forge-redis:/data/dump.rdb ./backups/redis/

# Restore Qdrant from snapshot
docker cp ./backups/qdrant/snapshot.tar forge-qdrant:/qdrant/snapshots/
curl -X PUT http://localhost:6333/collections/forge_documents/snapshots/upload \
  -F "snapshot=@snapshot.tar"

# Restore Redis
docker cp ./backups/redis/dump.rdb forge-redis:/data/
docker restart forge-redis
```
...
docker exec forge-backend nvidia-smi
...
```yaml
services:
  forge-backend:    8000:8000  # REST API
...
<Callout type="warning">
**GPU Limitation**: Each forge-backend instance requires dedicated GPU access. Scale horizontally by deploying multiple GPU nodes with load balancer.
</Callout>
...
BACKUP_DIR=/backups/forge/$(date +%Y%m%d)
mkdir -p $BACKUP_DIR
...
docker exec forge-qdrant \
  curl -X POST http://localhost:6333/collections/forge_documents/snapshots
docker cp forge-qdrant:/qdrant/snapshots/ $BACKUP_DIR/qdrant/

# Backup Redis
echo "Backing up Redis..."
docker exec forge-redis redis-cli BGSAVE
sleep 5
docker cp forge-redis:/data/dump.rdb $BACKUP_DIR/redis/
...
docker volume rm forge_qdrant_storage
docker volume create forge_qdrant_storage
docker cp backup-20250128/qdrant/ forge-qdrant:/qdrant/storage/
