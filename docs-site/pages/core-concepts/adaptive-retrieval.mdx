...
description: Learn how Forge intelligently adapts retrieval strategies based on query complexity for optimal speed-accuracy balance
---
...
Forge features an intelligent **adaptive retrieval system** that automatically selects the optimal search strategy based on query complexity. The system balances speed and accuracy by choosing between three distinct modes: Simple, Hybrid, and Advanced.

## Overview

Traditional RAG systems use a single retrieval approach for all queries, leading to either slow performance or poor accuracy. Forge's adaptive retrieval solves this by:
...
When `mode: "adaptive"` is specified, Forge uses LLM-based query classification to automatically select the optimal strategy.
...
Forge's adaptive retrieval system provides:
