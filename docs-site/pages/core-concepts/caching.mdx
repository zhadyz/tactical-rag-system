...
description: "Comprehensive guide to Forge's five-layer Forge caching protocol for 98% latency reduction"
---

import { Callout } from '@/components/Callout'

# Forge Multi-Level Caching

Forge employs a sophisticated **five-layer caching architecture** (L1-L5) called the Forge that achieves **98% latency reduction** for cached queries, dropping response times from 50-100ms to less than 1ms.

## Caching Overview

### Why Forge Matters

The Forge (Adaptive Tiered Latency-Aware Storage) protocol is designed to minimize redundant computation at every stage of the RAG pipeline:
...
When the conversation exceeds 5 exchanges, Forge automatically summarizes older context to stay within token limits:
...
```dockerfile
# Stage 3: Pre-cache HuggingFace models (backend/Dockerfile)
...
**Production Benefit**: L4 caching combined with parallel initialization reduces Forge's startup time from 78 seconds to **20-30 seconds**.
</Callout>
...
Learn about **[Adaptive Retrieval](/core-concepts/adaptive-retrieval)** to understand how Forge intelligently routes queries to different retrieval strategies based on complexity.
