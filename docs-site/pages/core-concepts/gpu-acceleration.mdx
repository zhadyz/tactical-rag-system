...
description: Learn how Forge leverages GPU acceleration for 80-100 tokens/sec inference with CUDA and llama.cpp
---
...
Forge achieves **80-100 tokens/sec** inference speeds by leveraging GPU acceleration through CUDA 12.1 and llama.cpp. This page explains how the system utilizes GPU resources for optimal performance.
...
Forge requires **CUDA 12.1** for GPU acceleration. The system is optimized for modern NVIDIA GPUs with compute capability 8.0+.
...
Forge uses **llama.cpp** via the `llama-cpp-python` bindings for direct GPU acceleration, bypassing Ollama's HTTP overhead.
...
<Callout type="info">
**Thread Safety**: llama.cpp is NOT thread-safe. Forge uses a single-thread executor with semaphore to prevent race conditions.
</Callout>
...
<Callout type="error">
**PyTorch Compatibility**: RTX 5080 has sm_120 architecture incompatibility with PyTorch 2.5.1. Forge forces embeddings to CPU as a workaround.
</Callout>
...
# Stream GPU stats
docker exec forge-backend watch -n 1 nvidia-smi
```

### Performance Metrics API

Forge exposes GPU metrics via the health endpoint:
