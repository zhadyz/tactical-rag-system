...
Forge supports multiple installation methods depending on your use case and infrastructure.
...
<Callout type="info">
  Forge works on CPU-only systems but GPU acceleration provides **10x faster** retrieval and
...
The fastest way to get Forge running is with Docker Compose:
...
Install Forge as a Python package:
...
# Install Forge with GPU support
pip install forge-rag[gpu]

# Or CPU-only version
pip install forge-rag
...
```bash
# For PDF processing
pip install forge-rag[pdf]

# For advanced monitoring
pip install forge-rag[monitoring]

# Install all extras
pip install forge-rag[all]
```
...
<Callout type="warning">
  NVIDIA GPUs are not supported on modern macOS. Forge can run on CPU or you can use Docker with
...
If CUDA is installed but Forge doesn't detect it:
...
pip cache purge
pip install --force-reinstall forge-rag[gpu]
```
...
- [Configure](/getting-started/configuration) your deployment
- [Upload your first document](/getting-started/first-query)
- [Explore the API](/api-reference)

<Callout type="info">
  Need help? Join our [Discord community](https://discord.gg/forge-rag) or check the
