...
description: Learn how to execute queries in Forge and understand the response format
---
...
- Forge backend is running (`docker compose up -d`)
...
The simplest way to query Forge is using **Simple Mode**, which performs direct dense vector search without query transformations.
...
curl -X POST http://localhost:8000/api/query \
  ...
  -d '{
    "question": "What are the key features of Forge?",
    "mode": "simple",
    "use_context": true
  }'
```
...
Forge supports three retrieval modes, each optimized for different query types:
...
```json
{
  "question": "Compare the caching strategies in Forge",
  "mode": "adaptive"
}
```
...
curl -X POST http://localhost:8000/api/query \
  -H "Content-Type: application/json" \
  -d '{
    "question": "How does Forge achieve sub-second cache hits while maintaining high accuracy?",
    "mode": "adaptive",
    "use_context": true,
...
Forge processes queries through a sophisticated 26-stage pipeline:
