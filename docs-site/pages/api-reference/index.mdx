---
title: API Reference
description: Complete Forge API documentation with interactive examples
---
...
Forge provides a REST API for document retrieval and question answering. All endpoints return JSON and support CORS.
...
Currently, Forge runs without authentication for development. For production deployments, implement authentication middleware or use a reverse proxy like Nginx.

<Callout type="warning">
  **Security:** Enable rate limiting and authentication before deploying to production. Forge includes built-in rate limiting (30 requests/minute per IP).
</Callout>
...
| `version` | string | Forge version number |
...
async function queryForge(question: string): Promise<QueryResponse> {
...
// Usage
const result = await queryForge("Can I grow a beard?");
...
Forge enforces rate limiting to prevent abuse:
...
Forge automatically detects and logs potential prompt injection attempts:
...
Forge includes multi-layer caching:
...
When GPU is available, Forge automatically:
...
def query_forge_safe(question: str, mode: str = "simple"):
    try:
        response = requests.post(
            "http://localhost:8000/api/query",
...
# Usage
result = query_forge_safe("Can I grow a beard?")
if result:
    print(result['answer'])
```
...
<Callout type="success">
  **Ready to integrate Forge?** Check out the [interactive demos](/interactive-demos) to explore Forge's capabilities.
</Callout>

## Next Steps

- **[Quick Start](/getting-started/quick-start)** - Get Forge running in 5 minutes
- **[Architecture](/architecture/overview)** - Understand how Forge works
