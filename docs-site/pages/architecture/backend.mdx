...
description: Deep dive into Forge's FastAPI backend, multi-stage caching, and GPU-accelerated inference engine
---
...
The Forge backend is a **FastAPI-based Python system** with 8 architectural layers, GPU-accelerated inference via llama.cpp, and a sophisticated multi-stage caching protocol (Forge L1-L5). It achieves **80-100 tokens/sec** inference speed and **less than 1ms** cache hit latency.
...
- **Caching**: L1-L5 Forge protocol (multi-stage optimization)
...
| **2. RAG Engine** | Component orchestration, parallel init | `app/core/rag_engine.py` | 3.4x startup speedup (78s → 23s) |
...
Forge supports **Server-Sent Events (SSE)** for real-time token streaming:
...
<Callout type="tip">
**SSE vs WebSocket**: Forge uses SSE for unidirectional streaming (server → client). SSE is simpler, auto-reconnects, and works better with HTTP/2 multiplexing than WebSockets.
</Callout>
...
Forge uses **llama.cpp** directly (not Ollama) for **8-10x faster inference**:
...
## Forge Caching Protocol

### Multi-Stage Architecture (L1-L5)

The Forge protocol implements **5 cache layers** for maximum performance:
...
Forge uses **BAAI/bge-large-en-v1.5** (1024-dimensional) for dense vector embeddings:
...
Forge leverages **CUDA 12.1** for GPU-accelerated inference:
...
<Callout type="info">
**ChromaDB Fallback**: Forge maintains ChromaDB support for backward compatibility. Qdrant is recommended for production due to 10x better performance at scale.
</Callout>
...
Forge processes multiple document formats with specialized extractors:
...
| Parallel Initialization | 3.4x (78s → 23s) | `asyncio.gather()` groups |
| Embedding Cache (L4) | 98% (50ms → less than 1ms) | Redis with pickle serialization |
| BGE Reranker | 85% (400ms → 60ms) | GPU-accelerated BAAI model |
...
Continue exploring Forge's architecture:
