...
description: Step-by-step guide to reproduce Forge performance benchmarks
---
...
This guide provides detailed instructions to reproduce Forge's benchmark results independently. Follow these steps to verify the reported performance metrics on your own hardware.
...
Clone the Forge repository with benchmark scripts:
...
# Checkout v5.0 production branch
...
wget https://benchmark-data.forge-rag.io/wiki-100k.tar.gz
...
docker build -f Dockerfile -t forge-backend:benchmark .

# Verify build (should be ~9GB)
docker images forge-backend:benchmark
```
...
Launch all services (Qdrant, Redis, Forge backend):
...
docker-compose -f docker-compose.yml logs -f forge-backend
```
...
docker-compose logs -f forge-backend | grep "Processing"
```
...
docker exec forge-backend python -c "import torch; print(torch.cuda.is_available())"
```
...
docker-compose logs forge-backend | grep "FORCE_TORCH_CPU"
...
- **Share Results**: Submit your benchmark results via [GitHub Discussions](https://github.com/yourusername/forge-rag/discussions)
...
- **Docker Compose Reference**: `backend/docker-compose.yml`
