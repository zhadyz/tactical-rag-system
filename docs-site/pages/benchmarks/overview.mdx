...
description: Forge performance benchmarks and comparisons
---
...
Forge delivers exceptional performance through GPU acceleration and intelligent retrieval strategies.
...
<Callout type="success">
  Forge achieves **10x faster** retrieval than CPU-only systems while maintaining higher accuracy
...
Forge's P95 latency of **127ms** is 7x faster than LangChain and 5x faster than LlamaIndex. This
includes:
...
Forge handles **450 queries/second** with concurrent processing, compared to:
...
Forge achieves **94.2% accuracy** on context relevance benchmarks through:
...
Forge achieves **88% GPU utilization** by:
...
Unlike other frameworks, Forge runs **every** compute-intensive operation on GPU:

```python
# Forge: Everything on GPU
...
Forge analyzes query complexity and adjusts retrieval strategy:
...
Forge batches operations for maximum GPU efficiency:
...
git clone https://github.com/yourusername/forge-benchmarks.git
cd forge-benchmarks
